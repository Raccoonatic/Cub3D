...
/*   Updated: 2026/09/08 01:09:08 by lde-san-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cb_minimap_compose(t_game *g, t_mp *m)
{
	...
	c.th = g->mp.ph.h;
	c.tw = g->mp.ph.w;
	c.x = 30 + (g->ply.ren_x - g->mp.x) - (c.tw / 2);
	c.y = (g->h - g->mp.h - 30) + (g->ply.ren_y - g->mp.y) - (c.th / 2);
...
{
	g->mp.x = g->ply.ren_x - (g->mp.w / 2);
	g->mp.y = g->ply.ren_y - (g->mp.h / 2);
	if (g->mp.x < 0)
		g->mp.x = 0;
	if (g->mp.y < 0)
		g->mp.y = 0;
